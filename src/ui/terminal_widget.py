...
class InteractiveTerminal(QTextEdit):
    ...
    # 送ると行が無くなる文字。行送り（CR / LF）と、行全体を捨てる Ctrl+C (0x03)。
    # Ctrl+U (0x15) はここに入れない。機器側の Ctrl+U が消すのはカーソルより
    # 前だけで（bash は unix-line-discard、Cisco IOS も「カーソルから行頭まで」）、
    # Ctrl+A や矢印で行頭側へ移ってから押すと機器には行が残る。しかもここへ
    # 入れると、直前の矢印キーが立てた打ちかけの印まで解いてしまっていた
    # （実測: 'echo CHECK' → Ctrl+A → Ctrl+U でキープアライブの CR が送られ、
    # 'echo CHECK' が実行された）。行が残っているか判断できない操作として
    # 打ちかけのまま扱う（利用者の決定 2026-09-20）
    LINE_ENDERS = ("\r", "\n", "\x03")
    ...
    def _note_typed(self, payload: str) -> None:
        ...
        判断できない操作（Backspace で消した、矢印で動かした、Ctrl+U で
        カーソルより前を消した、など）は打ちかけのままにして、キープ
        アライブを送らない側へ倒す。
