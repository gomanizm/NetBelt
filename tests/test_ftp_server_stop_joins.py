...
のは待受スレッド自身の役目になったため（別スレッドから閉じると
select が読んでいる最中の fd リストを書き換えることになり、Windows
では poll の中でプロセスごと落ちる。tests/
test_ftp_server_close_on_serving_thread.py を参照）、居残りはこの
終了処理の中で起きる。
"""
import os
import sys
