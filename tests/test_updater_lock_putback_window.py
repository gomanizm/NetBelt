...
def _gated_updater():
    """関門を 4 か所（A 用 2・B 用 2）入れた updater.bat を返す。"""
...
class UpdaterLockPutBackWindowTest(unittest.TestCase):
    ...
    def _start(self, tag, swap=None, sweep=None, stale=None, grab=None):
        ...
        env["TEMP"] = self.temp
        env["TMP"] = self.temp
        for name in ("NB_TEST_GATE", "NB_GATE_SWEEP", "NB_GATE_STALE",
                     "NB_GATE_GRAB"):
            env.pop(name, None)
        if swap:
