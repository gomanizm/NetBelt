...
直し方: 行を動かす前に、範囲の下端の印を外す。印は行と一緒に動くので、
動いた先 (scroll_bottom - n) で外すのと同じ結果になる。最下行での
折り返し (_linefeed(from_wrap=True)) の印だけはそこに載っているので、
その経路だけ _scroll_up(from_wrap=True) で除外する。

はじめは「動いた先が範囲の外へ出た (押し出されて履歴へ入った) ときは
触らない」としていたが、それでは範囲の高さぶん回したときに印ごと履歴へ
入り、そのあと空いた先頭へ来た無関係な出力と 1 行に繋がる。そちらは
tests/test_terminal_pushed_out_wrap_mark.py で別に見ている。印が範囲の
下端そのものに無い場合 (下の test_scrolling_the_whole_region_away_...)
は、これまでどおり印ごと履歴へ入る。
"""
import os
import sys
