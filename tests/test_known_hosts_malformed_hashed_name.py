...
- 自前ローダ（load_known_hosts）を通る場合（読めない行・BOM・cp932 で
  ...
  AssertionError('')（base64 として読めない名前では binascii.Error）になる。
  利用者に出るのは『接続エラー: 』だけ（本文は空か『Incorrect padding』）で、
  どの接続先でも同じ。どの行が原因かも分からない。
- paramiko の HostKeys.load を通る場合: 崩れた行より後に行があれば、
  load 内の check() で同じ例外になり「known_hosts を読めないため接続を
  中止しました」（理由は空か『Incorrect padding』）で全機器が止まる。
  崩れた行が最後の行なら load は通り、上と同じく connect で
  『接続エラー: 』になる。
